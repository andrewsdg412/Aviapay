...
var ui = {
    ...
    initCardPayment: function(card_num, exp_date) {
        ...
        */
        ...
        if (blacklistManager.checkNumberExists(card_num)) { // don't process if the card is blacklisted.
            ui.cardPaymentFailed("Problem authorizing card, please try another card or cash.");
            return;
        }
    ...
    getCrewMembers: function() {
        return amplify.store('crew_members');
    },
    ...
    showTransTotal: function(trans, is_credit_note) {
        var total = amplify.store("default_currency_symbol") + " "  + ui.getTransTotal(trans, is_credit_note).toFixed(2);
    ...
    showShiftSummary: function(shift) {
        ...
        $.each(completed, function(idx, trans) {
            if (trans.shift_id == shift.id) {
                console.log(trans.shift_id + " == " + shift.id);
                console.log(trans);
                $.each(trans.products, function(prod_idx, prod) {
                    if (trans.payment_type == "cc") {
                        if (trans.failed) {
                            total_failed+= parseFloat(prod.price) * prod.qty;
                        } else {
                            total_card+= parseFloat(prod.price) * prod.qty;
                        }
                    } else if (trans.payment_type == "cash") {
                        total_cash+= parseFloat(prod.price) * prod.qty;
                    }
                });
            }
        });
        ...
        var id = appNav.pushTemplate("shift_summary_tpl", {
            ...
            flight_number: shift.flight_number,
...
//M1DAWSON JOHN MR              LGWKBPPS 0112 263C002A0001  00
    ...
    init: function() {
        ...
        //Check for updates on init as well as every 3 hours.
        ui.checkForUpdate();
        setInterval(function() {
            ui.checkForUpdate();
        }, 3 * 60 * 60 * 1000);
...
var toImg = function toImg(target) {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            html2canvas($('#canvas_frame').contents().find('body'), {
                    onrendered: function(canvas) {
                        document.body.appendChild(canvas);
                        //var canvas = document.getElementById('');
