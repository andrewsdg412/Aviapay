...
reports = {
    ...
    _renderShift: function(shift, current_shift) {
        var html = '<tr><td>';
        html += '<br><b>Flight Number: </b>';
        html += this._clean(shift.flight_number);
        html += '<br><br><b>Crew: </b><br>&nbsp;&nbsp;';
        html += this._clean(shift.crew[0].name + '<br>&nbsp;&nbsp;' + shift.crew[1].name);
    ...
    showShiftTransSummary: function(shift_id) {
        ...
        function populateTrans() {
            ...
            $.each(product_bulk_totals, function(prod_id, total) {
                var comb = total.cash + total.cc_pending + total.cc_success + total.cc_failed;
                var prod = ui.getProduct(prod_id);
                html_bulk_arr.push(
                    "<tr>" +
            ...
            $('#shift_trans_bulk_summary_list').html(html_bulk_arr.join(''));
